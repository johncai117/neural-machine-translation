...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        #self.encoder = nn.LSTM(embed_size, int(hidden_size/2), bidirectional=True)
        ...
        self.att_projection2 = nn.Linear(hidden_size, hidden_size, bias=False)

        self.att_projectionv = nn.Linear(hidden_size, 1, bias=False)

        # Linear Layer with no bias), called W_{u} in the PDF.
        #self.combined_output_projection = nn.Linear(hidden_size + hidden_size, hidden_size, bias=False)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        target_len = target_padded.size(0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #temp = torch.bmm(dec_hidden, enc_hiddens_proj)
        #DOT PRODUCT
        #temp_enc = enc_hiddens.permute(0,2,1)
        #dec_hidden = torch.unsqueeze(dec_hidden, dim = 1)
        #temp = torch.bmm(dec_hidden, temp_enc)
        #ADDITIVE ATTENTION
        dec_hiddens_proj = self.att_projection2.forward(dec_hidden)
        dec_hiddens_proj = torch.unsqueeze(dec_hiddens_proj, dim = 1)
        temp_dec_enc = dec_hiddens_proj + enc_hiddens_proj
        temp_tanh = torch.tanh(temp_dec_enc)
        temp = (self.att_projectionv.forward(temp_tanh)).permute(0,2,1)
        #COMPUTING e_t
        e_t = torch.squeeze(temp, dim = 1)
        #       Hints:
        ...
        temp_alpha = torch.unsqueeze(alpha_t, dim = 1)
        temp_a = torch.bmm(temp_alpha, enc_hiddens)
        a_t = torch.squeeze(temp_a, dim = 1)
        #     Hints:
        ...
        temp_O = torch.tanh(V_t)
        O_t = self.dropout(temp_O)
        # Use the following docs to implement this functionality:
        #     Softmax:
    ...
    def beam_search(self, src_sent: List[str],
        ...
        src_encodings_att_linear = self.att_projection.forward(src_encodings)
